...
For Multiaxis Intelligence (www.multiaxis.ai)
Created by: Michael H Kaminski Jr., CEO of Multiaxis LLC

A complete quiz generation system with GUI interface.
Run with: python quiz_generator_app.py
"""
...
# Initialize LOG_FILE before the try block
LOG_FILE = "quiz_generator.log"  # Default fallback
...
try:
    ...
    LOG_FILE = os.path.join(log_dir, f"quiz_generator_{timestamp}.log")
    ...
    # Use the fallback LOG_FILE defined above
    logging.basicConfig(
        level=logging.DEBUG,
        format='%(asctime)s - %(levelname)s - %(funcName)s - %(message)s',
        filename=LOG_FILE
    )
    logging.error(f"Error setting up logging directory: {e}")

# ============= EMBEDDED SAMPLE QUIZZES =============
...
class QuizGenerator:
    ...
    def generate_html(self, output_file: str):
        ...
        try:
            ...
            logo_base64 = ""
            logo_path = "quiz_generator_app_logo.png"
            try:
        ...
        <body>
            <div class="certificate">
                ...
                <div class="footer">
                    ...
                    <div class="signature">
                        <div class="signature-line"></div>
                        <div>Michael Kaminski<br>CEO, Multiaxis LLC</div>
                    </div>
                </div>
                <div class="meta">
    ...
    <div class="certificate-container">
        ...
        <div class="certificate-footer">
            ...
            <div class="signature-block">
                ...
                <div class="signature-title">CEO, Multiaxis LLC</div>
            </div>
        </div>
...
class QuizGeneratorApp:
    """GUI Application for Quiz Generator."""
    ...
    def __init__(self, root):
        ...
        try:
            ...
            if os.path.exists('quiz_generator_app.png'):
                icon = tk.PhotoImage(file='quiz_generator_app.png')
    ...
    def load_settings(self):
        ...
        self.settings = {
            'company_name': 'Multiaxis Intelligence',
            'author': '',
        ...
        try:
            if os.path.exists('quiz_settings.json'):
    ...
    def create_widgets(self):
        ...
        self.title_var = tk.StringVar(value="Interactive Knowledge Quiz")
    ...
    def edit_question(self):
        ...
        question_data = self.quiz_gen.questions[self.current_question_index]
        
        # Create edit dialog
        edit_window = tk.Toplevel(self.root)
        edit_window.title(f"Edit Question {self.current_question_index + 1}")
        edit_window.geometry("700x600")
        
        # Question
        ttk.Label(edit_window, text="Question:").grid(row=0, column=0, sticky=tk.W, padx=10, pady=5)
        q_text = tk.Text(edit_window, height=3, width=60)
        q_text.grid(row=0, column=1, padx=10, pady=5)
        q_text.insert('1.0', question_data['question'])
        
        # Options
        option_vars = []
        for i, opt_label in enumerate(['A', 'B', 'C', 'D']):
            ttk.Label(edit_window, text=f"Option {opt_label}:").grid(row=i+1, column=0, sticky=tk.W, padx=10, pady=5)
            var = tk.StringVar(value=question_data['options'][i] if i < len(question_data['options']) else "")
            entry = ttk.Entry(edit_window, textvariable=var, width=60)
            entry.grid(row=i+1, column=1, padx=10, pady=5)
            option_vars.append(var)
        
        # Correct answer
        ttk.Label(edit_window, text="Correct Answer:").grid(row=5, column=0, sticky=tk.W, padx=10, pady=5)
        correct_var = tk.StringVar(value=chr(65 + question_data['correct']))
        correct_frame = ttk.Frame(edit_window)
        correct_frame.grid(row=5, column=1, sticky=tk.W, padx=10, pady=5)
        for i, letter in enumerate(['A', 'B', 'C', 'D']):
            ttk.Radiobutton(correct_frame, text=letter, variable=correct_var, value=letter).grid(row=0, column=i, padx=5)
        
        # Explanation
        ttk.Label(edit_window, text="Explanation:").grid(row=6, column=0, sticky=tk.W, padx=10, pady=5)
        exp_text = tk.Text(edit_window, height=3, width=60)
        exp_text.grid(row=6, column=1, padx=10, pady=5)
        exp_text.insert('1.0', question_data.get('explanation', ''))
        
        # Image
        ttk.Label(edit_window, text="Image filename:").grid(row=7, column=0, sticky=tk.W, padx=10, pady=5)
        image_var = tk.StringVar(value=question_data.get('image', ''))
        ttk.Entry(edit_window, textvariable=image_var, width=60).grid(row=7, column=1, padx=10, pady=5)
        
        # Difficulty
        ttk.Label(edit_window, text="Difficulty:").grid(row=8, column=0, sticky=tk.W, padx=10, pady=5)
        diff_var = tk.StringVar(value=question_data.get('difficulty', 'Medium'))
        diff_combo = ttk.Combobox(edit_window, textvariable=diff_var, values=["Easy", "Medium", "Hard"], state="readonly", width=20)
        diff_combo.grid(row=8, column=1, sticky=tk.W, padx=10, pady=5)
        
        def save_changes():
            # Update question data
            new_question = q_text.get('1.0', tk.END).strip()
            new_options = [var.get().strip() for var in option_vars if var.get().strip()]
            ...
            if len(new_options) < 2:
                messagebox.showerror("Error", "At least 2 options required")
                return
    ...
    def paste_text(self):
        """Open dialog to paste formatted text."""
        dialog = tk.Toplevel(self.root)
        dialog.title("Paste Quiz Questions")
        dialog.geometry("600x500")
        
        ttk.Label(dialog, text="Paste formatted questions (use --- to separate questions):").pack(pady=5)
...
Q: Next question here..."""
        ...
        ttk.Button(dialog, text="Process Text", command=process_text).pack(pady=10)
    ...
    def preview_current_question(self):
        ...
        if self.current_question_index < 0 or self.current_question_index >= self.quiz_gen.get_question_count():
            messagebox.showinfo("No Selection", "Please select a question to preview")
            return
        
        # Preview is already shown in main window, just highlight it
        self.update_preview()
        messagebox.showinfo("Preview", f"Question {self.current_question_index + 1} is shown in the preview area below.")
    ...
    def show_add_question_dialog(self):
        ...
        # Question
        ttk.Label(add_window, text="Question:").grid(row=0, column=0, sticky=tk.W, padx=10, pady=5)
        q_text = tk.Text(add_window, height=3, width=60)
        q_text.grid(row=0, column=1, padx=10, pady=5)
        
        # Options
        option_vars = []
        for i, opt_label in enumerate(['A', 'B', 'C', 'D']):
            ttk.Label(add_window, text=f"Option {opt_label}:").grid(row=i+1, column=0, sticky=tk.W, padx=10, pady=5)
            var = tk.StringVar()
            entry = ttk.Entry(add_window, textvariable=var, width=60)
            entry.grid(row=i+1, column=1, padx=10, pady=5)
            option_vars.append(var)
        
        # Correct answer
        ttk.Label(add_window, text="Correct Answer:").grid(row=5, column=0, sticky=tk.W, padx=10, pady=5)
        correct_var = tk.StringVar(value="A")
        correct_frame = ttk.Frame(add_window)
        correct_frame.grid(row=5, column=1, sticky=tk.W, padx=10, pady=5)
        for i, letter in enumerate(['A', 'B', 'C', 'D']):
            ttk.Radiobutton(correct_frame, text=letter, variable=correct_var, value=letter).grid(row=0, column=i, padx=5)
        
        # Explanation
        ttk.Label(add_window, text="Explanation:").grid(row=6, column=0, sticky=tk.W, padx=10, pady=5)
        exp_text = tk.Text(add_window, height=3, width=60)
        exp_text.grid(row=6, column=1, padx=10, pady=5)
        
        # Image
        ttk.Label(add_window, text="Image filename:").grid(row=7, column=0, sticky=tk.W, padx=10, pady=5)
        image_var = tk.StringVar()
        image_entry = ttk.Entry(add_window, textvariable=image_var, width=60)
        image_entry.grid(row=7, column=1, padx=10, pady=5)
        ttk.Label(add_window, text="(Optional - e.g., q1.png)", foreground="gray").grid(row=7, column=2, padx=5)
        
        # Auto-suggest image name
        suggested_name = f"q{self.quiz_gen.get_question_count() + 1}.png"
        ttk.Label(add_window, text=f"Suggested: {suggested_name}", foreground="gray").grid(row=8, column=1, sticky=tk.W, padx=10)
        
        # Difficulty
        ttk.Label(add_window, text="Difficulty:").grid(row=9, column=0, sticky=tk.W, padx=10, pady=5)
        diff_var = tk.StringVar(value="Medium")
        diff_combo = ttk.Combobox(add_window, textvariable=diff_var, values=["Easy", "Medium", "Hard"], state="readonly", width=20)
        diff_combo.grid(row=9, column=1, sticky=tk.W, padx=10, pady=5)
        ...
        def add_question():
            ...
            if not question:
                self.show_notification("Please enter a question", "error")
                return
            
            if len(options) < 2:
                self.show_notification("Please enter at least 2 options", "error")
                return
            ...
            # Update status and set to new question
            self.current_question_index = self.quiz_gen.get_question_count() - 1
            self.update_status()
            
            status_msg = f"Question {self.quiz_gen.get_question_count()} added ({diff_var.get()})"
            if image_var.get().strip():
                status_msg += f" with image"
            self.show_notification(status_msg, "success")
            add_window.destroy()
        
        # Buttons
        button_frame = ttk.Frame(add_window)
        button_frame.grid(row=10, column=0, columnspan=2, pady=20)
        ttk.Button(button_frame, text="Add Question", command=add_question).grid(row=0, column=0, padx=10)
    ...
    def preview_questions(self):
        ...
        preview.title("Preview Questions")
        ...
        for i, q in enumerate(self.quiz_gen.questions, 1):
            text_widget.insert(tk.END, f"Question {i}:\n", "heading")
            text_widget.insert(tk.END, f"{html.unescape(q['question'])}\n\n")
        
            for j, option in enumerate(q['options']):
                marker = " ✓" if j == q['correct'] else ""
                text_widget.insert(tk.END, f"  {chr(65+j)}) {html.unescape(option)}{marker}\n")
    ...
    def view_log(self):
        """Open and display the current log file."""
        try:
            log_window = tk.Toplevel(self.root)
            log_window.title(f"Log File - {os.path.basename(self.log_file)}")
            ...
            ttk.Label(toolbar, text=f"Log file: {self.log_file}").pack(side=tk.LEFT, padx=5)
            ...
            ttk.Button(toolbar, text="Refresh", command=refresh_log).pack(side=tk.RIGHT, padx=5)
            ttk.Button(toolbar, text="Open Log Folder", command=open_log_folder).pack(side=tk.RIGHT, padx=5)
...
AI PROMPT FOR GENERATING FILES:
...
www.multiaxis.ai"""
        ...
        help_window.title("Help - Quiz Generator")
        help_window.geometry("700x600")
        
        # Create scrolled text widget
        help_text_widget = scrolledtext.ScrolledText(help_window, wrap=tk.WORD, width=80, height=35)
...
# ============= MODIFY THE create_widgets METHOD =============
    ...
    def show_about(self):
        ...
        about_text = """Quiz Generator
Version 1.0

Created for Multiaxis Intelligence
www.multiaxis.ai

Developed for:
Michael H. Kaminski Jr.
CEO, Multiaxis LLC
www.multiaxis.llc

Advisory: Methods Machine Tools
...
knowledge assessments and training materials."""
        ...
        about_window.title("About Quiz Generator")
        about_window.geometry("600x700")
        
        # Create scrolled text widget
        about_text_widget = scrolledtext.ScrolledText(about_window, wrap=tk.WORD, width=70, height=35)
        about_text_widget.pack(padx=10, pady=10, fill=tk.BOTH, expand=True)
